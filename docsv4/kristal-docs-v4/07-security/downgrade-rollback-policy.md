...
This policy is required to make **signed Runtime Packs** and Exchange release pointers safe to distribute at scale, especially in offline or intermittently connected environments.
...
* Key lifecycle procedures (generation/rotation) beyond what is required for enforcement (see key-management doc)
* Full content trust model beyond signature verification + pinned trust roots
...
* **Runtime Pack**: derived, offline-executable distribution payload (bundle) identified by `runtime_pack_id` (content-addressed, preferred) plus `pack_version` (monotonic per channel).
...
* Operator mistakes pushing the wrong version to a region/audience.

This policy assumes:

* Signature verification exists and is **fail-closed** where signatures/integrity are declared.
* Trust roots are **pinned per channel** and can be verified offline.
* Revocation data MAY or MAY NOT be available offline; the policy MUST handle both cases.
* Runtime Pack distribution is offline-friendly (bundles and indexes may be cached for long periods).

---

## Cryptographic and integrity baseline (normative)

### Supported signature algorithms (minimum set)

An implementation MUST support:

* **ed25519** signatures (required baseline for v3)

An implementation MAY support additional algorithms via profiles (e.g., RSA-PSS-SHA256) but they MUST NOT replace the required baseline.

### Runtime Pack signing scope (required)

A Runtime Pack MUST be distributable as an integrity-protected bundle:

* The **pack manifest** MUST be signed.
* The pack manifest MUST include hashes for all payload files it declares, and consumers MUST verify these hashes when verification is enabled/configured.

This ensures offline caches cannot silently substitute payload files while keeping the manifest signature valid.

### Hash field naming (normative)

Where a hash object is used, it MUST use:

* `alg` (NOT `algo`) and `value`

---

## Required metadata for enforcement

### A) Runtime Pack identifiers (mandatory)

Each Runtime Pack bundle MUST be uniquely identified by:

* `runtime_pack_id` (content-addressed, preferred) OR deterministic `pack_content_id`
* `exchange_ref.exchange_id` (the Exchange `kristal_id` used to compile the pack)
* `build.build_id` (compiler build id)
...
* optional `region` and/or `audience_segment`
...
* `channel_id` (tenant/env[/region][/audience])
* `latest` pointer(s)
* `pinned` pointer(s) (optional)
* optional `minimum_allowed_version`
* optional `revoked` list (or reference to one)
* optional staged pointers/cohorts (canary)
* optional `reissue_allowed` and/or `reissue_flags` (see below)
* signature(s) and signer identity

### D) Offline revocation data (optional but supported)

If an offline revocation artifact is provided (example: `revocations.json`, content-addressed and signed), then:

* clients MUST treat it as policy input for activation decisions
* clients MUST fail-closed if the channel policy requires revocation checking but no current revocation data is available offline

### E) Timestamps and signatures (normative constraints)
...
* last successful verification metadata (e.g., signer key id, index verification state, revocation-check state)
...
* verify pack manifest signature(s)
* verify all declared payload file hashes (if present/configured)
...
If a client receives a pack with the same `pack_version` but a different `runtime_pack_id`:
...
**Reissue rules (minimum):**
* If reissue is permitted, the signed index MUST bind the reissue to a target `runtime_pack_id` (or list of allowed ids) and provide a reason.
* Clients MUST log reissue activations.

### 6) Revocation-aware blocking (if revocations are provided)

If a verified channel index includes a `revoked` list (or references an offline revocations artifact):
...
* optional `target_runtime_pack_id` (recommended, to prevent ambiguity)
* `reason`
* `issued_at`
...
* `PACK_VERIFIED` (runtime_pack_id, pack_version, signer_key_id)
* `PACK_REJECTED` (reason: verification_failed | downgrade_blocked | revoked | index_signature_failed | reissue_not_allowed)
* `DOWNGRADE_BLOCKED`
...
6. Pack integrity covers declared payload files (manifest signature + file hashes).
...
* Whether revocation artifacts are mandatory for specific channels (e.g., prod) and what the freshness rules are offline.